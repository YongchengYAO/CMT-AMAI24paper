function faces_out = cal_surfaceDilation_restricted(...
    faces_in,...
    faces_source,...
    vers_source,...
    vers_border)
...
% ----------------------------------------------------
% edges
edges_source = cal_Faces2Edges(faces_source);
% neighboring faces for each edge
[neighFaces_source, ~] = cal_neighFaces4Edges(faces_source);
% ----------------------------------------------------


%% Find the first layer of faces to be added
% delete duplicated faces
faces_in = remove_duplicated_triangles(faces_in);
% detect surface edge of the input mesh
[marginEdges_in, ~] = cal_detectSurfaceBoundary(faces_in);
...
% (!!!important!!!) ---
faces_tba = remove_triangles(faces_in, faces_tba, 'explicit'); % MeshProcessingToolbox
% (!!!important!!!) ---
...
if ~isempty(faces_tba)
    while flag_continue
        % detect surface edge of the input mesh
        [marginEdges_tba, ~] = cal_detectSurfaceBoundary(faces_tba);
        ...
        % (!!!important!!!) ---
        faces_tba_next = remove_triangles(faces_in, faces_tba_next, 'explicit'); % MeshProcessingToolbox
        faces_tba_next = remove_triangles(faces_tba, faces_tba_next, 'explicit'); % MeshProcessingToolbox
